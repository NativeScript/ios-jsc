...
static void enableDebugging(int argc, char **argv) {
  ...
  TNSInspectorFrontendConnectedHandler connectionHandler =
      ^TNSInspectorProtocolHandler(
           TNSInspectorSendMessageBlock sendMessageToFrontend,
           NSError * error) {
    if (error) {
      if (listenSource) {
    ...
    return ^(NSString * message, NSError *error) {
              if (message) { CFRunLoopRef runloop = CFRunLoopGetMain();
    CFRunLoopPerformBlock(runloop, (__bridge CFTypeRef)(inspectorRunloopModes),
    ...
    if (error) {
      NSLog(@"NativeScript debugger received %@. Disconnecting.", error);
    } else {
...
notify_register_dispatch(NOTIFICATION("AttachRequest"),
                         &attachRequestSubscription, dispatch_get_main_queue(),
                         ^(int token) {
  if (listenSource) {
    return;
  }
