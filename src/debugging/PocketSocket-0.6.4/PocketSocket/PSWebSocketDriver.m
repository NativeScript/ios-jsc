...
+ (BOOL)isWebSocketRequest:(NSURLRequest*)request {
    ...
    if (headers[@"Sec-WebSocket-Key"] &&
        ...
        [request.HTTPMethod.lowercaseString isEqualToString:@"get"] && request.HTTPBody.length == 0) {
        return YES;
    }
    return NO;
}
...
+ (instancetype)clientDriverWithRequest:(NSURLRequest*)request {
    return [[self alloc] initWithMode:PSWebSocketModeClient
                              request:request];
}
+ (instancetype)serverDriverWithRequest:(NSURLRequest*)request {
    return [[self alloc] initWithMode:PSWebSocketModeServer
                              request:request];
}
...
- (void)sendText:(NSString*)text {
    ...
    [self writeMessageWithOpCode:PSWebSocketOpCodeText
                            data:data];
}
- (void)sendBinary:(NSData*)binary {
    [self writeMessageWithOpCode:PSWebSocketOpCodeBinary
                            data:binary];
}
- (void)sendCloseCode:(NSInteger)code reason:(NSString*)reason {
    ...
    [self writeMessageWithOpCode:PSWebSocketOpCodeClose
                            data:data];
}
- (void)sendPing:(NSData*)data {
    [self writeMessageWithOpCode:PSWebSocketOpCodePing
                            data:data];
}
- (void)sendPong:(NSData*)data {
    [self writeMessageWithOpCode:PSWebSocketOpCodePong
                            data:data];
}
...
- (void)writeHandshakeRequest {
    ...
    [_request.allHTTPHeaderFields enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        CFHTTPMessageSetHeaderFieldValue(msg, (__bridge CFStringRef)[
...
- (void)writeMessageWithOpCode:(PSWebSocketOpCode)opcode data:(NSData*)data {
    ...
    if (_pmdEnabled && !PSWebSocketOpCodeIsControl(opcode) && [payload length] > 0) {
        ...
        if ((_pmdClientNoContextTakeover && _mode == PSWebSocketModeClient) || (_pmdServerNoContextTakeover && _mode == PSWebSocketModeServer)) {
            [_deflater reset];
        }
    ...
    if (_mode == PSWebSocketModeClient) {
        ...
        [header appendBytes:maskKey
                     length:sizeof(maskKey)];
...
- (NSInteger)readBytes:(void*)bytes maxLength:(NSUInteger)maxLength error:(NSError* __autoreleasing*)outError {
    ...
    case PSWebSocketDriverStateFramePayload: {
        ...
        if (_pmdEnabled && !frame->control && (frame->rsv1 || _initialFrame->rsv1)) {
            ...
            if ((_pmdClientNoContextTakeover && _mode == PSWebSocketModeServer) || (_pmdServerNoContextTakeover && _mode == PSWebSocketModeClient)) {
                [_inflater reset];
            }

            // begin the inflater
            if (frame->payloadLength == frame->payloadRemainingLength) {
                if (![_inflater begin:frame->buffer
                                error:outError]) {
                    return -1;
                }
            }
            ...
            // inflate bytes
            if (![_inflater appendBytes:bytes
        ...
        // otherwise append
            ...
            [frame->buffer appendBytes:bytes
                                length:consumeLength];
        }
...
- (BOOL)processFramesAndDelegate:(NSError* __autoreleasing*)outError {
    ...
    case PSWebSocketOpCodeBinary:
        [_delegate driver:self
            didReceiveMessage:frame->buffer];
        break;
    case PSWebSocketOpCodePong:
        [_delegate driver:self
            didReceivePong:frame->buffer];
        break;
    case PSWebSocketOpCodePing: {
        [_delegate driver:self
            didReceivePing:frame->buffer];
        break;
    }
...
- (BOOL)pmdConfigureWithExtensionsHeaderComponents:(NSArray*)components {
    ...
    if (_mode == PSWebSocketModeClient) {
        ...
        _deflater = [[PSWebSocketDeflater alloc] initWithWindowBits:_pmdServerWindowBits
                                                        memoryLevel:8];
    } else {
        _inflater = [[PSWebSocketInflater alloc] initWithWindowBits:_pmdServerWindowBits];
        _deflater = [[PSWebSocketDeflater alloc] initWithWindowBits:_pmdClientWindowBits
                                                        memoryLevel:8];
    }
...
- (NSString*)acceptHeaderForKey:(NSString*)key {
    ...
    data = [NSData dataWithBytes:sha1
