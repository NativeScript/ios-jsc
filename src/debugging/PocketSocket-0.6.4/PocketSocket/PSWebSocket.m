...
- (instancetype)initClientSocketWithRequest:(NSURLRequest*)request {
    if ((self = [self initWithMode:PSWebSocketModeClient
                           request:request])) {
...
#endif
            [_outputStream setProperty:opts
...
+ (instancetype)serverSocketWithRequest:(NSURLRequest*)request inputStream:(NSInputStream*)inputStream outputStream:(NSOutputStream*)outputStream {
    return [[self alloc] initServerWithRequest:request
...
- (instancetype)initServerWithRequest:(NSURLRequest*)request inputStream:(NSInputStream*)inputStream outputStream:(NSOutputStream*)outputStream {
    if ((self = [self initWithMode:PSWebSocketModeServer
                           request:request])) {
...
- (void)connect {
    ...
    // schedule streams
    [_inputStream scheduleInRunLoop:[[self class] runLoop]
                            forMode:NSDefaultRunLoopMode];
    [_outputStream scheduleInRunLoop:[[self class] runLoop]
                             forMode:NSDefaultRunLoopMode];
...
- (void)disconnect {
    ...
    [_inputStream removeFromRunLoop:[[self class] runLoop]
                            forMode:NSDefaultRunLoopMode];
    [_outputStream removeFromRunLoop:[[self class] runLoop]
                             forMode:NSDefaultRunLoopMode];
...
- (void)pumpInput {
    ...
    @autoreleasepool {
        ...
        while (_inputStream.hasBytesAvailable) {
            NSInteger readLength = [_inputStream read:chunkBuffer
                                            maxLength:sizeof(chunkBuffer)];
            if (readLength > 0) {
                if (!_inputBuffer.hasBytesAvailable) {
                    NSInteger consumedLength = [_driver execute:chunkBuffer
                    ...
                    if (consumedLength < readLength) {
                        ...
                        [_inputBuffer appendBytes:chunkBuffer + offset
                                           length:remaining];
                    }
                } else {
                    [_inputBuffer appendBytes:chunkBuffer
                                       length:readLength];
                }
        ...
        while (_inputBuffer.hasBytesAvailable) {
            NSInteger readLength = [_driver execute:_inputBuffer.mutableBytes
                                          maxLength:_inputBuffer.bytesAvailable];
            if (readLength <= 0) {
                break;
            }
...
- (void)pumpOutput {
    ...
    while (_outputStream.hasSpaceAvailable && _outputBuffer.hasBytesAvailable) {
        NSInteger writeLength = [_outputStream write:_outputBuffer.bytes
                                           maxLength:_outputBuffer.bytesAvailable];
        if (writeLength <= -1) {
            _failed = YES;
            [self disconnect];
            NSString* reason = @"Failed to write to output stream";
            NSError* error = [NSError errorWithDomain:PSWebSocketErrorDomain
                                                 code:PSWebSocketErrorCodeConnectionFailed
                                             userInfo:@{
    ...
    if (_closeWhenFinishedOutput && !_outputBuffer.hasBytesAvailable && (_inputStream.streamStatus != NSStreamStatusNotOpen && _inputStream.streamStatus != NSStreamStatusClosed) && !_sentClose) {
        _sentClose = YES;

        [self disconnect];

        if (!_failed) {
            [self notifyDelegateDidCloseWithCode:_closeCode
                                          reason:_closeReason
                                        wasClean:YES];
        }
    }
...
- (void)failWithCode:(NSInteger)code reason:(NSString*)reason {
    ...
    [self failWithError:[NSError errorWithDomain:PSWebSocketErrorDomain
                                            code:code
                                        userInfo:userInfo]];
}
...
- (void)driverDidOpen:(PSWebSocketDriver*)driver {
    if (_readyState != PSWebSocketReadyStateConnecting) {
        [NSException raise:@"Invalid State"
                    format:@"Ready state must be connecting to become open"];
        return;
    }
