    ...
    if ((self = [super init])) {
        ...
        if (host && ![host isEqualToString:@"0.0.0.0"]) {
            ...
            if (!addr.sin_addr.s_addr) {
                [NSException raise:@"Invalid host"
        ...
        _addrData = [NSData dataWithBytes:&addr
                                   length:sizeof(addr)];
...
- (void)attachConnection:(PSWebSocketServerConnection*)connection {
    ...
    [_connectionsByStreams setObject:connection
                              forKey:connection.inputStream];
    [_connectionsByStreams setObject:connection
                              forKey:connection.outputStream];
...
- (void)pumpInput {
    ...
    for (PSWebSocketServerConnection* connection in _connections.allObjects) {
        if (connection.readyState != PSWebSocketServerConnectionReadyStateOpen || !connection.inputStream.hasBytesAvailable) {
            continue;
        }
        ...
        while (connection.inputStream.hasBytesAvailable) {
            ...
            if (readLength > 0) {
                [connection.inputBuffer appendBytes:chunkBuffer
                                             length:readLength];
        ...
        if (connection.inputBuffer.bytesAvailable > 4) {
            ...
            // create webSocket
            PSWebSocket* webSocket = [PSWebSocket serverSocketWithRequest:request
...
- (void)pumpOutput {
    for (PSWebSocketServerConnection* connection in _connections.allObjects) {
        if (connection.readyState != PSWebSocketServerConnectionReadyStateOpen && connection.readyState != PSWebSocketServerConnectionReadyStateClosing) {
            continue;
        }

        while (connection.outputStream.hasSpaceAvailable && connection.outputBuffer.hasBytesAvailable) {
            NSInteger writeLength = [connection.outputStream write:connection.outputBuffer.bytes
        ...
        if (connection.readyState == PSWebSocketServerConnectionReadyStateClosing && !connection.outputBuffer.hasBytesAvailable) {
            [self disconnectConnection:connection];
        }
    }
}
