...
- (void)performTest:(XCTestRun*)run {
    ...
    for (NSUInteger i = 1; i <= run.test.testCaseCount; ++i) {
        ...
        XCTestCase* testCase = [[[self class] alloc] initWithInvocation:invocation
                                                                   desc:desc];
...
- (NSDictionary*)autobahnPerformTestCaseNumber:(NSInteger)number {
    ...
    NSURL* URL = [NSURL URLWithString:extra
                        relativeToURL:self.autobahnURL];
...
- (NSDictionary*)autobahnFetchTestCaseInfoForNumber:(NSUInteger)number {
    ...
    NSDictionary* info = [NSJSONSerialization JSONObjectWithData:[op.message dataUsingEncoding:NSUTF8StringEncoding]
                                                         options:0
                                                           error:nil];
