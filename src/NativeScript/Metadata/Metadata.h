...
struct Array {
    ...
    T* first() const { return(T*)(&count + 1); }
    
    T& operator[](int index) const { return *(first() + index); }
    
    Array<T>::iterator begin() const { return first(); }
    
    Array<T>::iterator end() const { return first() + count; }
    
    template <typename V> const Array<V>& castTo() const { return *reinterpret_cast<const Array<V>*>(this); }
...
public:
    ...
    const GlobalTable *globalTable() const { return &this->_globalTable; }
...
struct PtrTo {
    ...
    PtrTo<T> operator+(int value) const { return add(value); }
    T *operator->() const { return valuePtr(); }
    PtrTo<T> add(int value) const { return PtrTo<T> { .offset = this->offset + value * sizeof(T) }; }
    PtrTo<T> addBytes(int bytes) const { return PtrTo<T> { .offset = this->offset + bytes }; }
    template<typename V> PtrTo<V>& castTo() const { return reinterpret_cast<PtrTo<V>>(this); }
    T* valuePtr() const { return isNull() ? nullptr : (T*)((char *)MetaFile::instance()->heap() + this->offset); }
...
union TypeEncodingDetails {
    struct IncompleteArrayDetails {
        TypeEncoding *getInnerType() { return (TypeEncoding *)this; }
    } incompleteArray;
    struct ConstantArrayDetails {
        int32_t size;
        TypeEncoding *getInnerType() { return (TypeEncoding *)(this + 1); }
    } constantArray;
    ...
    struct AnonymousRecordDetails {
        uint8_t fieldsCount;
        String *getFieldNames() { return (String *)(this + 1); }
        TypeEncoding *getFieldsEncodings() { return (TypeEncoding *)(getFieldNames() + this->fieldsCount); }
    } anonymousRecord;
};
